...
`v0.1.0` 到 `v0.7.25` 及后续 `Unreleased` 的主要变化。
...
- 交叉构建改为支持单架构入口，CI 与 Release 分别并行构建七个架构，减少串行等待。
- Release 在工具未变化时复用上一正式版本的工具包，并将 ECS 主程序构建与组装解耦。
- 固定 Go 1.27.1 为源码 canonical formatter，统一本地格式化、质量检查和贡献指南入口。

## 0.7.29 — 2026-09-06

- 修复 cgroup 祖先资源限制、挂载与命名空间路径解析，以及内存使用量和可用量的层级计算。
- 修复 HTTP 正文截断误判、NextTrace 输出通道混流、外部命令进程组取消和输出缓冲上限。
- 修复下载传输时限与依赖环境刷新，收敛评分数值、比较渲染、工具目录、执行计划和 render 默认输出。
- 精简架构检查、发布打包和 CI 触发流程，固定 GitHub Actions 引用，并保留冻结工具校验与真实集成门禁。

## 0.7.26 — 2026-09-04

- 删除 benchmark 的自动二次执行与候选结果选择，移除旧的判定与报告结构；窗口系统状态继续作为同一次测试的普通 `Measurements`，schema 仍为 `ecs.report/v1`。

## 0.7.25 — 2026-09-04

- 统一运行时基准工具来源：按本次 `ecs plan` 选择，从匹配架构的已冻结工具包临时 staging，只从该目录执行，不再回退到用户 PATH。
- 固定工具包准备失败或关闭自动依赖时直接终止运行，不生成工具缺失回退报告；移除不再独立提供价值的 `ecs doctor` 和系统 benchmark 安装入口。
- 收紧运行脚本、探针工具查找和相关回归测试，保留 Release 校验、临时目录隔离与报告输出流程。
- CI 集成验证同步通过显式临时 staging 提供已安装的真实工具，生产探针与集成测试的工具来源契约保持一致。
