...
- 排行榜提交新增 `fingerprint_version: "v2"`。v2 指纹覆盖除服务端 ID 与运行时间外的全部可接受公开字段，并保留浮点数精度，避免同一测量因时间不同无法去重，也避免旧算法按三位小数舍入造成碰撞；缺少版本的历史记录继续使用冻结的旧算法，未知版本明确拒绝。
...
- 增加覆盖恶意终端序列、全报告 IP 泄漏、恶意节点清单/私网 DNS/重定向、单核真实执行次数、指纹兼容、稀疏 Tier 指标、跨运营商回程以及严格 manifest 合同的回归测试。
