...
usage() {
  printf '%s\n' \
    ...
    "sysbench, fio, iperf3 and traceroute through the detected system package manager."
}
...
as_root() {
  if [ "$(id -u)" -eq 0 ]; then
    ...
    printf '%s\n' "benchmark dependencies require root; re-run as root or install sysbench fio iperf3 traceroute manually" >&2
    exit 1
  fi
}
...
install_benchmark_tools() {
  ...
  printf '%s\n' "installing standard benchmark tools: sysbench fio iperf3 traceroute"
  if command -v apt-get >/dev/null 2>&1; then
    as_root env DEBIAN_FRONTEND=noninteractive apt-get update
    as_root env DEBIAN_FRONTEND=noninteractive apt-get install -y sysbench fio iperf3 traceroute
  elif command -v dnf >/dev/null 2>&1; then
    as_root dnf install -y sysbench fio iperf3 traceroute
  elif command -v yum >/dev/null 2>&1; then
    as_root yum install -y sysbench fio iperf3 traceroute
