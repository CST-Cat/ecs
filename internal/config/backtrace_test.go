...
func TestBacktraceTargetsFor(t *testing.T) {
	...
	// 四城市各三大运营商。
	if len(all) != 12 {
		t.Fatalf("全部城市应有 12 个目标，实际 %d", len(all))
	}
	carriers := map[string]int{}
	seen := map[string]bool{}
	for _, target := range all {
		if net.ParseIP(target.Address) == nil {
			t.Fatalf("回程目标必须是 IP 字面量（避免解析结果随 DNS 漂移）：%q", target.Address)
		}
	...
	for _, carrier := range []string{"电信", "联通", "移动"} {
		if carriers[carrier] != 4 {
			t.Errorf("%s 应有 4 个目标（每城市一个），实际 %d", carrier, carriers[carrier])
		}
	}

	// 顺序必须按 BacktraceCityOrder 固定，不能随 map 遍历漂移。
	first := BacktraceTargetsFor([]string{"chengdu", "beijing"})
	if len(first) != 6 || first[0].Kind != "电信" || first[0].Name != "北京电信" {
...
func TestDefaultsUsesBeijingAndGuangzhou(t *testing.T) {
	...
	if len(cfg.BacktraceTargets) != 6 {
		t.Fatalf("默认应为北京+广州共 6 个目标，实际 %d", len(cfg.BacktraceTargets))
	}
