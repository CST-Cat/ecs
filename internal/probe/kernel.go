...
// Kernel network parameters retain stable logical keys. Platform-specific
// paths and readers live in *_linux.go and *_freebsd.go files.
...
func appendKernelNetworkFacts(result *model.Result, values map[string]string) {
	...
	if platformKernelRmemMaxAvailable() {
		if raw, ok := values["rmem_max"]; ok {
			if bufferBytes, err := strconv.Atoi(raw); err == nil && bufferBytes > 0 {
				result.Measurements = append(result.Measurements, model.Measurement{
					...
					Method: kernelRmemMethod(), HigherIsBetter: model.BoolPtr(true),
				})
