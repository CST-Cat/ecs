...
// Sysbench's text parser and result assembly are shared by Linux and FreeBSD;
// only the load/fact collector is platform-specific.
func TestSysbenchParserAndMeasurements(t *testing.T) {
	...
	if !rateOK || !eventsOK || !p95OK || rate != 1234.5 || events != 2469 || p95 != 1.25 {
		t.Fatalf("portable sysbench parse = rate:%v/%v events:%v/%v p95:%v/%v", rate, rateOK, events, eventsOK, p95, p95OK)
	}
	if _, ok := parseFirstFloat(sysbenchP95Pattern, "95th percentile: invalid\n"); ok {
	...
	if formatSysbenchEvents(sysbenchCPUResult{}) != "unavailable" || formatSysbenchEvents(sysbenchCPUResult{Rate: 1, Events: 2}) != "2" {
		t.Fatal("portable sysbench event formatting failed")
	}
