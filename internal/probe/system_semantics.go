...
func buildSystemResult(start time.Time, snapshot systemSnapshot, resources EnvironmentSnapshot, cloud cloudIdentity) model.Result {
	...
	result.Fields = []model.Field{
		...
		systemField("cpu_topology", systemCPUTopologyValue(snapshot)),
		...
		systemField("memory_total", systemMemoryTotalValue(snapshot)),
		systemField("memory_used", systemMemoryUsedValue(snapshot)),
		systemField("memory_available", systemMemoryAvailableValue(snapshot)),
		systemField("memory_usage_percent", systemMemoryUsageValue(snapshot)),
		...
		systemField("swap", systemSwapValue(snapshot)),
		...
		systemField("disk_total", systemDiskTotalValue(snapshot)),
		systemField("disk_used", systemDiskUsedValue(snapshot)),
		systemField("disk_available", systemDiskFreeValue(snapshot)),
		systemField("disk_usage_percent", systemDiskUsageValue(snapshot)),
	...
	result.SummaryMessages = []model.Message{model.NewMessage(
		...
		systemMemoryTotalValue(snapshot),
		systemDiskFreeValue(snapshot),
...
func systemCPUTopologyValue(snapshot systemSnapshot) string {
	physical := "unknown"
	if snapshot.PhysicalCoresKnown && snapshot.PhysicalCores > 0 {
		physical = strconv.Itoa(snapshot.PhysicalCores)
	}
	return fmt.Sprintf("logical=%d;physical=%s", snapshot.LogicalCPUs, physical)
}
...
func systemMemoryTotalValue(snapshot systemSnapshot) string {
	if !snapshot.MemoryTotalKnown {
		return "unavailable"
	}
	return model.FormatBytes(snapshot.MemoryTotal)
}

func systemMemoryUsedValue(snapshot systemSnapshot) string {
	if !snapshot.MemoryUsedKnown {
		return "unavailable"
	}
	return model.FormatBytes(snapshot.MemoryUsed)
}

func systemMemoryAvailableValue(snapshot systemSnapshot) string {
	if !snapshot.MemoryAvailableKnown {
		return "unavailable"
	}
	return model.FormatBytes(snapshot.MemoryFree)
}

func systemMemoryUsageValue(snapshot systemSnapshot) string {
	if !snapshot.MemoryUsedKnown || !snapshot.MemoryAvailableKnown {
		return "unavailable"
	}
	return fmt.Sprintf("%.1f %%", snapshot.MemoryUsage)
}

func systemSwapValue(snapshot systemSnapshot) string {
	if !snapshot.SwapKnown {
		return "unavailable"
	}
	return model.FormatBytes(snapshot.SwapTotal)
}

func systemDiskTotalValue(snapshot systemSnapshot) string {
	if !snapshot.DiskKnown {
		return "unavailable"
	}
	return model.FormatBytes(snapshot.DiskTotal)
}

func systemDiskUsedValue(snapshot systemSnapshot) string {
	if !snapshot.DiskKnown {
		return "unavailable"
	}
	return model.FormatBytes(snapshot.DiskUsed)
}

func systemDiskFreeValue(snapshot systemSnapshot) string {
	if !snapshot.DiskKnown {
		return "unavailable"
	}
	return model.FormatBytes(snapshot.DiskFree)
}

func systemDiskUsageValue(snapshot systemSnapshot) string {
	if !snapshot.DiskKnown {
		return "unavailable"
	}
	return fmt.Sprintf("%.1f %%", snapshot.DiskUsage)
}

func appendSystemResourceFields(fields *[]model.Field, limits resourceLimits) {
	*fields = append(*fields,
		systemField("cgroup_cpu_quota", systemCgroupCPUValue(limits.CPU)),
		...
		systemField("cgroup_memory_limit_bytes", systemMemoryLimitMachineValue(limits)),
...
func stableSystemMeasurements(snapshot systemSnapshot, resources EnvironmentSnapshot) []model.Measurement {
	...
	if snapshot.MemoryTotalKnown {
		method := systemMemoryMeasurementMethod(snapshot)
		measurements = append(measurements, systemMeasurement("memory_total_bytes", float64(snapshot.MemoryTotal), "bytes", model.FormatBytes(snapshot.MemoryTotal), method, nil))
		if snapshot.MemoryUsedKnown {
			measurements = append(measurements, systemMeasurement("memory_used_bytes", float64(snapshot.MemoryUsed), "bytes", model.FormatBytes(snapshot.MemoryUsed), method, model.BoolPtr(false)))
		}
		if snapshot.MemoryAvailableKnown || snapshot.MemoryFree > 0 {
			measurements = append(measurements,
				systemMeasurement("memory_available_bytes", float64(snapshot.MemoryFree), "bytes", model.FormatBytes(snapshot.MemoryFree), method, model.BoolPtr(true)),
				systemMeasurement("memory_usage_percent", snapshot.MemoryUsage, "%", fmt.Sprintf("%.1f %%", snapshot.MemoryUsage), method, model.BoolPtr(false)),
			)
		}
	}
	if snapshot.DiskKnown {
	...
	if snapshot.MemoryLimit > 0 && platformPressureFactsAvailable() {
...
func systemResourceMeasurements(resources EnvironmentSnapshot) []model.Measurement {
	// The common renderer knows Linux resource method IDs, but FreeBSD has no
	// cgroup/PSI source.  Keep this guard at the final emission boundary so a
	// malformed or injected FreeBSD snapshot can never serialize Linux facts.
