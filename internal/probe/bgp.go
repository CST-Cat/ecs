...
func (bgpProbe) Run(ctx context.Context, env Environment) model.Result {
	...
	addComparisonParameter(result.Methodology.Parameters, "provider", "routeviews-current-rib")

	result.Sources = []model.Source{{
		Name:    "RouteViews API",
		URL:     routeViewsAPI + "/docs/",
		Purpose: "probe.bgp.source.routeviews",
	}}
	table := model.Table{
		Key:   "network.bgp.observation",
		Title: "probe.bgp.table.observation",
		Columns: []model.TableColumn{
			{Key: "ip_family", Label: "probe.bgp.column.ip_family"},
			{Key: "matched_prefix", Label: "probe.bgp.column.prefix"},
			{Key: "origin_asn", Label: "probe.bgp.column.origin_asn"},
			{Key: "rpki_state", Label: "probe.bgp.column.rpki"},
			{Key: "peer_collector", Label: "probe.bgp.column.peer_collector"},
			{Key: "as_path_sample", Label: "probe.bgp.column.as_path"},
		},
	...
	for _, version := range versions {
		...
		result.Fields = append(result.Fields,
			model.Field{Key: "ipv" + version + "_ip", Label: bgpFieldLabel("ipv" + version + "_ip"), Value: model.RawValue(egressIP), Sensitive: true},
		)
		// ASN 只有走情报接口时才有。STUN 路径下留空而不是填 unknown：
		// RouteViews 返回的起源 ASN 本来就是这一项的权威来源。
		if intel, ok := env.Egress.IntelFor(version); ok {
			result.Fields = append(result.Fields,
				model.Field{Key: "ipv" + version + "_ipapi_asn", Label: bgpFieldLabel("ipv" + version + "_ipapi_asn"), Value: model.RawValue(formatASN(intel.ASN.ASN))},
			)
		}
		...
		if err != nil {
			...
			result.Fields = append(result.Fields, model.Field{
				Key: "ipv" + version + "_routeviews_error", Label: bgpFieldLabel("ipv" + version + "_routeviews_error"), Value: model.RawValue(compactError(err)),
			})
			continue
		}
		...
		for index, observation := range observations {
			...
			if index == 0 {
				result.Fields = append(result.Fields,
					model.Field{Key: "ipv" + version + "_prefix", Label: bgpFieldLabel("ipv" + version + "_prefix"), Value: model.RawValue(observation.Prefix)},
			...
			table.Rows = append(table.Rows, []model.Value{
				model.RawValue("IPv" + version),
				model.RawValue(observation.Prefix),
				model.RawValue(formatASN(observation.OriginASN)),
				model.RawValue(fallback(observation.RPKIState, "unknown")),
				model.RawValue(peerText),
				model.RawValue(pathText),
			})
		}
	}
	...
	result.SummaryMessages = []model.Message{model.NewMessage("probe.bgp.summary.values", successes)}
	result.Finish(start)
	return result
}

func bgpFieldLabel(key string) string {
	if strings.HasPrefix(key, "ipv") {
		return "probe.bgp.field.ip_family"
	}
	return "probe.bgp.field." + key
}
