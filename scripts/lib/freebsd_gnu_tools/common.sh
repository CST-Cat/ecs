...
#
# Each final binary is stripped once with the SDK's target strip
# (${triple}-strip), then rechecked against the static FreeBSD ELF contract.
# Release-artifact verification and the real FreeBSD tools gate cover the
# remaining tree-level and runtime behavior.
# ---------------------------------------------------------------------------
...
ecs_freebsd_gnu_strip_release_binaries() {
  ...
  for tool in npb-ep npb-ft stream; do
    ...
    # Recheck architecture/FreeBSD identity/static/no NEEDED/no GLIBC_.
