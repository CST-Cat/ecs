...
# 下载纪律：每次尝试 --connect-timeout 30 --max-time 60、最多 3 次，禁用
...
#
# 每次尝试限时（--connect-timeout 30 --max-time 60）、最多 3 次；禁用
# curl --retry 的指数退避——NASA 持续 500 期间它曾把 CI 烧掉 35 分钟
# （run 34757858241）。SHA-256 与钉值不匹配是官方渠道异常（不是网络失败），
# 直接终止构建，不重试也不静默换源。
ecs_freebsd_gnu_npb_download() {
  ...
  for attempt in 1 2 3; do
    echo "npb: official NASA download attempt $attempt/3 (max-time 60s)" >&2
