...
私有互联或完整历史。`ookla` 是单独的显式外部适配器：不会自动下载客户端，也不会在普通配置档中运行；
只有用户同时选择模块并接受条款后，官方客户端才会执行真实测速。Ookla 可能接收测量所需的出口 IP、
客户端和服务器元数据，ecs 只提取选择后的字段，不能把该模式表述为零上传。
...
在线模块仍会让目标服务看到发起请求的出口 IP；完整列表见 README 的“隐私与网络请求”章节。使用 `--offline` 可禁止这些模块运行。
