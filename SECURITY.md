...
`ecs` 的原生探针不需要 root。默认不会安装软件、修改内核参数、更新包管理器、写系统目录或上传报告。

磁盘模块只在用户指定目录创建名称随机的 `.ecs-fio-*` 临时文件，并在完成、错误或取消时清理。它会把文件限制在测试前可用空间的 20% 以内。

路由模块只调用机器上已经存在的 `nexttrace`、`traceroute`、`tracepath` 或 `tracert`，参数以数组传递，不经过 shell。当前版本不会自动下载这些程序，并在报告中记录参数与程序 SHA-256。NextTrace 只使用无启动横幅的 JSON 输出模式。
