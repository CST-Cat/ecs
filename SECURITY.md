...
磁盘模块只在用户指定的测试目录创建随机命名的 `.ecs-fio-*` 文件，文件大小不超过测试前可用空间的 20%，并在成功、错误或取消时清理。它只运行 `PATH` 中已有或由 `run.sh` 临时提供的 `fio`，解析本地 JSON 输出并记录程序版本和完整参数。
...
路由与回程模块只使用官方 NextTrace Tiny，以参数数组调用无启动横幅的 JSON 模式，不经过 shell，并记录实际版本和完整参数。缺少工具时，`run.sh` 先校验当前架构 `ecs-tools` 归档在 Release `checksums.txt` 中的摘要，再只把本次需要的成员 staging 到私有 `$WORK/bin`；失败或 `ECS_AUTO_DEPS=0` 时跳过相关模块，退出时清理 `$WORK`，不安装到系统。
