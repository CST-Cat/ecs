...
The Windows frozen bundle contains seven tools: `zstd`, NPB EP, NPB FT, OpenSSL, STREAM, `fio`, and the official NextTrace Tiny v1.7.1 release asset. The Windows AMD64 prebuilt `nexttrace-tiny_windows_amd64.exe` must be verified against locked SHA-256 `16e13532f6e8ee75f63db61a6a98fe1ca217b5431b76531c8c5d4bcdbe7e6f9b` and copied byte-for-byte into the Bundle; the other six tools remain the frozen benchmark workloads and the fio gate requires and verifies `windowsaio`. `sysbench`, `iperf3` and Ookla are unsupported on Windows. Windows `route` and `backtrace` use the same canonical NextTrace arguments, JSON adapter and production parser as Linux, and record engine, version, adapter, target, family, hop facts and tool provenance. The final Windows Server 2022/2025 workflow must run a genuine IPv4 gate on both runners and must run IPv6 when a global IPv6 address and default route are available; otherwise it reports the missing capability explicitly. Release publication waits for the final Windows rehearsal and uses neither `tracert.exe` nor a host-PATH fallback.
