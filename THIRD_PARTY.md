...
- [VirusTotal](https://www.virustotal.com/)（需 `VIRUSTOTAL_API_KEY`）：数十家安全厂商对该 IP 的投票统计，语义是"判为恶意的厂商占比"，与单模型欺诈分不可直接比较；
- [ipgeolocation.io](https://ipgeolocation.io/)（需 `IPGEOLOCATION_API_KEY`）：威胁分与代理/Tor/云厂商信号；
- [BigDataCloud](https://www.bigdatacloud.com/)（需 `BIGDATACLOUD_API_KEY`）：hazardReport 的代理/Tor/VPN/Spamhaus DROP 信号；免密端点实测返回 403 配额超限；
- [getipintel.net](https://getipintel.net/)（需 `GETIPINTEL_CONTACT`）：代理/VPN 概率。其服务条款要求请求附带联系邮箱，且对数据中心 IP 段设限——实测 DigitalOcean 出口直接被拒；邮箱只从环境变量读取，用户未提供则不发请求；
