...
| CPU 单线程/多线程固定工作负载 | — | sysbench CPU（唯一） | 标准短测 | 标准 | 标准长测 |
| 内存顺序读写带宽 | — | sysbench memory（唯一） | 标准短测 | 标准 | 标准长测 |
| 磁盘顺序、随机与同步延迟 | — | fio JSON（可用） | 标准短测 | 标准 | 标准长测 |
| DNS 延迟、失败率与抖动 | ✓ | — | ✓ | ✓ | ✓ |
| TCP 延迟与可达率 | ✓ | ICMP ping（规划） | ✓ | ✓ | ✓ |
| 多节点上传/下载吞吐 | — | iperf3 JSON（唯一，逐节点原值） | — | 标准 | 标准长测 |
| 常用及邮件端口出站能力 | ✓ | — | — | ✓ | ✓ |
| 流媒体与 AI 服务区域检测 | ✓ | 社区规则包（规划） | — | ✓ | ✓ |
