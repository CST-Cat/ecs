...
4. **标准性能工具唯一**：所有配置档的 CPU、内存、磁盘和网络吞吐成绩分别调用 sysbench、fio、iperf3。项目不保留自研性能分数、自动回退、并行效率、跨节点均值/中位数或综合跑分。
...
6. **资源预算可见**：`quick`、`standard`、`full` 三个配置在运行前给出预计耗时、临时磁盘占用和最大网络流量；任何网络压力测试都可单独关闭。
...
| 磁盘顺序、随机与 50/50 混合矩阵 | — | fio JSON，引擎探测回退 | 6 作业 | 8 作业 | 8 作业 |
| DNS 延迟、失败率与抖动 | ✓ | — | ✓ | ✓ | ✓ |
...
## 三档配置的区分度实测（2026-08-01）

有人问三档之间是否有足够区分度、是否有必要保留。用实测数据回答。
...
同一台裸机上用三档的采样时长各跑五次 sysbench CPU：

| 采样时长 | 均值 events/s | 标准差 | 变异系数 |
| ---: | ---: | ---: | ---: |
| 5 s | 775.0 | 3.48 | 0.45 % |
| 10 s | 771.7 | 2.85 | 0.37 % |
| 15 s | 766.4 | 0.47 | **0.06 %** |

15 秒窗口的离散度确实比 10 秒小一个数量级，但**均值只差 0.68 %**——
在裸机上，更长的采样带来的是更漂亮的重复性，而不是不同的结论。

需要强调这只代表裸机。突发性能机型（AWS t 系列、GCP e2、阿里突发实例）
的 burst credit 会在几十秒内耗尽，那里 5 秒与 15 秒测到的是两回事——
这正是采样窗口不能低于 10 秒的原因（见"首版之后的实测校准"第 4 条）。
该场景尚无实测样本，列入待验证。
...
- **standard 与 full 的区分也有必要，但此前的定位是错的**。
  ...
  这是模块扩充后没同步的过时文案；而"更长"暗示更准，实测显示裸机上收益微弱。
  `full` 的真正价值在**覆盖面**：三网测速、UDP 丢包与抖动、测速节点从 3 个增到 7 个。
  三项描述均已按此重写。
- 采样参数的差异予以保留：它对突发机型有意义，成本也只是时间。
