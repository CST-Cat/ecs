...
- **社区中转失效的应对**：`ipinfo.check.place` 当前对全部查询返回 403 与 Cloudflare 挑战页，
  MaxMind、AbuseIPDB、Scamalytics、ipdata 四个免密数据源全部不可用，`原生/广播 IP` 判定
  随之失效。代码行为正确（如实标失败、不拿别家分数顶替），文档已更新说明现状。
  待办：换出口确认是否全局下线；若确已下线，考虑把这四家标为"需要密钥"，
