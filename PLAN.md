...
- **8 Runner 动态文本迁移 — IN PROGRESS**。目标：runner 自生成 notices、skip/panic/retry 等不再写 canonical Chinese；输入：阶段 7 Message；范围：runner→model；产出：结构化 messages，外部错误/raw evidence 保持原样；验收：runner 无 i18n import 或源文本生成，相关 tests 更新。当前 CI 已暴露中间态问题：旧 `PrivacyNotice` 字段引用、report v2 误升版 fixture/constant、`model.go` gofmt；必须全部修复并重新跑 CI 后才可完成。
